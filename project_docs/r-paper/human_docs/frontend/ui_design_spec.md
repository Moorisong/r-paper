...
| :--- | :--- | :--- | :--- |
| `/` | `Home` | 홈 (메인 화면) | 롤링페이퍼 생성 양식 및 유효성 검사, 상단 하루상자 바로가기 배너 |
| `/paper/:slug` | `PaperView` | 롤링페이퍼 공유/상세 화면 | 롤링페이퍼 메시지 목록 조회, 메시지 작성 모달, 링크 복사 유도 시스템, 상단 하루상자 바로가기 배너 |
| `*` | `NotFound` | 404 에러 화면 | 존재하지 않는 경로 요청 시 안내 및 홈 이동 |

---

## 🎁 하루상자 패밀리 서비스 연결 (`HarooBoxBanner`)
* **역할:** 롤링페이퍼를 방문한 유저가 '하루상자'(`https://box.haroo.site/`)의 다양한 콘텐츠를 자연스럽게 탐색할 수 있도록 제공하는 프리미엄 통합 안내 배너.
* **위치:** `PageContainer` 상단 영역에 고정 노출되어 모든 롤링페이퍼 페이지에서 일관되게 제공됨.
* **디자인:** 메인 카드와 일치하는 `glass-card` 테두리/그림자 및 `rounded-3xl` 곡률 적용, 넉넉한 텍스트 패딩 및 우측 이동 버튼 포함.
